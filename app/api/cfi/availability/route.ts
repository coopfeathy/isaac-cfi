...
export async function POST(request: NextRequest) {
  ...
  let body: { day_of_week: number; start_time: string; end_time: string }
  try {
  ...
  // Check for overlapping entries for this CFI on this day
...
export async function PATCH(request: NextRequest) {
  ...
  if (hasTimeChange) {
    // Fetch existing row (scoped to this CFI)
    ...
    // Check for overlapping entries (exclude self), scoped to this CFI
    const { data: overlaps } = await db
      .from('instructor_availability')
      .select('id')
      .eq('instructor_id', user.id)
...
export async function DELETE(request: NextRequest) {
  ...
  // Accept id from URL search params or body
